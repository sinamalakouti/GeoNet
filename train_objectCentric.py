...
from UDA_trainer.val import val_clip
...
def main():
    ...
    for it in range(start_it, cfg['training']['iteration']):
        ...
        trainer(batch_iterator, model, opt, it, device,cfg, logger, writer)

        if (it + 1) % cfg['training']['val_interval'] == 0:

            with torch.no_grad():
                acc_src, acc_src_top5 = val_clip(data_loader_src['test'], model, it, n_classes, logger, writer)

                acc_tgt, acc_tgt_top5 = val_clip(data_loader_tgt['test'], model, it, n_classes, logger, writer)
                is_best = False
                if acc_tgt > best_acc_tgt:
                    ...
                    with open(os.path.join(logdir, 'best_acc.txt'), "a") as fh:
                        write_str = "Source Top 1\t{src_top1:.3f}\tSource Top 5\t{src_top5:.3f}\tTarget Top 1\t{tgt_top1:.3f}\tTarget Top 5\t{tgt_top5:.3f}\n".format(
                            src_top1=best_acc_src, src_top5=best_acc_src_top5, tgt_top1=best_acc_tgt,
                            tgt_top5=best_acc_tgt_top5)
                        fh.write(write_str)

                print_str = '[Val] Iteration {it}\tBest Acc source. {acc_src:.3f}\tBest Acc target. {acc_tgt:.3f}'.format(
                    it=it + 1, acc_src=best_acc_src, acc_tgt=best_acc_tgt)
...
if __name__ == '__main__':
    ...
    valid_trainers = ["plain", "cdan", "mmd", "mmd_global_trainer", "object_centric_trainer"]
