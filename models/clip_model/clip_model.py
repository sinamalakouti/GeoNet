"""Module implementing support for pretrained clip_model models.
...
from models.utils import transformer_compute_positions, cnn_compute_positions_and_flatten

try:
    import clip
except ImportError:
    raise Exception("Using clip_model models requires installation with extra `clip_model`.")
...
class ClipImageModel(nn.Module):
    """Image part of pretrained clip_model model."""
    ...
    def __init__(
        ...
        Args:
            model_type: Model type matching `clip_model.load`.
    ...
    def apply_pooling(self, x):
        # apply attnpool on image features
        return self.clip_vision_model.attnpool(x)

    def forward(self, image):
        return self.forward_images(image)
    def forward_images(
            self, image
    ) -> Tuple:
        if self.freeze_model:
...
class ClipTextModel(nn.Module):
    """Text part of pretrained clip_model model."""
    ...
    def get_output(self, text):
        ...
        if self.remove_pooling:
            ...
            # Do what is done in the standard clip_model text encoder.
