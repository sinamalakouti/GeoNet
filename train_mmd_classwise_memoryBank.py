...
from UDA_trainer.mmd_classwise_memorybank_trainer import MemoryBank
...
def main():
    ...
    memory = MemoryBank(2048,(len(data_loader_src['train'].dataset), len(data_loader_tgt['train'].dataset)),0.9)
    for it in range(start_it, cfg['training']['iteration']):
        ...
        trainer(it, memory, batch_iterator, model_fe, model_cls, opt, it, device,cfg, logger, writer)
